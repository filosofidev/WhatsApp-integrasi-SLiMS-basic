...
echo simbio_form_element::dateField('untilDate', date('Y-m-d'),'class="form-control"');
  ...
  <?php
    ...
    function showOverduedList($obj_db, $array_data)
    {
        ...
        $_buffer .= '<div style="font-size: 10pt; margin-bottom: 3px;"><div id="' . $array_data[0] . 'emailStatus"></div>' . __('E-mail') . ': <a href="mailto:' . $member_d[1] . '">' . $member_d[1] . '</a> - <a class="usingAJAX" href="' . MWB . 'membership/overdue_mail.php' . '" postdata="memberID=' . $array_data[0] . '" loadcontainer="' . $array_data[0] . 'emailStatus">' . __('Send Notification e-mail') . '</a> - <a class="usingAJAX" href="' . MWB . 'membership/overdue_wa.php' . '" postdata="memberID=' . $array_data[0] . '" loadcontainer="' . $array_data[0] . 'emailStatus">' . __('Send Notification Whatsapp') . '</a> - ' . __('Phone Number') . ': ' . $member_d[2] . '</div>';
